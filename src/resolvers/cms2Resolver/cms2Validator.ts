...
export const cmsCreateValidator = [
    ...
    body('variables.input.items').optional({ checkFalsy: true }).isArray({ min: 0 }).custom((val) => {
        for (let item of val) {
            if (!item.title && !item.subTitle && !item.button && !item.button.buttonText && !item.button.redirectionURL) {
                return false
            }
        }
        return true;
...
export const cms2RecordByAdminQueryValidator = [
    body('variables.input._id').notEmpty().isMongoId(),
]