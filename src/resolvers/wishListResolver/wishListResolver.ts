...
export const wishListResolver: Resolvers = {
    ...
    Mutation: {
        ...
        addToMobileWishList: async (parent, { input }, { req }, info) => {
            try {

                await verifyMobileUser(req);
                await validateInput(validators.addToWishListMobileValidator, req);
                const userId = req.authAccount._id;
                const productId = input.productId;
        ...
        removeFromMobileWishList: async (parent, { input }, { req }, info) => {
            await verifyMobileUser(req);
            await validateInput(validators.removeFromWishListMobileValidator, req);
            const userId: Types.ObjectId = req.authAccount._id;
            const productId: Types.ObjectId = new Types.ObjectId(input.productId);
    ...
    Query: {
        ...
        getMobileWishListProducts: async (parent, { }, { req }, info) => {
            await verifyMobileUser(req);
            const userId: Types.ObjectId = req.authAccount._id;
        ...
        getMobileWishListProductStatus: async (parent, { input }, { req }, info) => {
            await verifyMobileUser(req);
            await validateInput(validators.wishListItemExistsMobileValidator, req);
            const userId: Types.ObjectId = req.authAccount._id;
            const productId = input.productId;
