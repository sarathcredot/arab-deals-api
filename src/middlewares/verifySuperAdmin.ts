...
export const verifySuperAdmin = async (req: CustomRequest) => {
  try {
    ...
    let admin = await adminService.findAdminWithFilters(
      { _id: decoded.id, token: token, isBlocked: false, accType: "SUPER_ADMIN" },
      { _id: 1, accType: 1 },
      { lean: true }
    );
    ...
    console.log(error);
    const errorMessage = error instanceof Error ? (error as Error).message : "An error occurred";
