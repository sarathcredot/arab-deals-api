...
import { MongoDBConnect, ApolloServerConnect, filePaths } from "./configs";
...
app.post(
  ...
  expressFileUpload({
    ...
    useTempFiles: false, // We'll handle buffers directly
  }),
  async (req, res) => {
    try {
      const { files } = req;
      ...
      for (const file of fileArray) {
        ...
        // Create a stream from the buffer if no temp file exists
        let fileStream;
        if (file.tempFilePath) {
          ...
          console.warn("Skipping file - no data or temp path available");
          continue;
        }

        const key = spaceService.getFileKey(filePath, file.name, []);

        const uploadedFile = await spaceService.publicFileUpload(
          key,
          file.mimetype,
      ...
      console.log(returnProductImage, "RETURN PRODUCT IMAGE");
      
      res.status(201).json({
        message: "Files uploaded successfully",
        data: returnProductImage,
      });
    } catch (error: any) {
